 ...
 * 便捷的错误创建函数
 */
export const createValidationError = (code, details) => ErrorHandler.handleValidationError(code, details);
export const createSystemError = (error, code) => ErrorHandler.handleSystemError(error, code);
export const showError = (error) => ErrorHandler.showError(error);
/**
 * 错误边界装饰器（用于类方法）
 */
export function withErrorHandling(errorCode = ERROR_CODES.TRANSPOSE_FAILED) {
