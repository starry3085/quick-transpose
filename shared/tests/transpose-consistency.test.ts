...
import { ERROR_CODES, ErrorHandler } from '../utils/error-handler';
import { CHORD_MAPS, ROMAN_NUMERALS } from '../constants/chord-data';
import type { TransposeParams, KeyType } from '../types/chord';
...
describe('Cross-platform Transpose Consistency Tests', () => {
  ...
  describe('Basic Transpose Functionality', () => {
    test('should transpose simple progression correctly', () => {
      const params: TransposeParams = {
        progression: '1 4 5 1',
        sourceKey: 'C' as KeyType,
        targetKey: 'G' as KeyType,
        isMinor: false
      };
    ...
    test('should handle minor key transposition', () => {
      const params: TransposeParams = {
        progression: '1 4 5 1',
        sourceKey: 'A' as KeyType,
        targetKey: 'D' as KeyType,
        isMinor: true
      };
  ...
  describe('All Keys Consistency', () => {
    const testKeys: KeyType[] = ['C', 'C#', 'Db', 'D', 'D#', 'Eb', 'E', 'F', 'F#', 'Gb', 'G', 'G#', 'Ab', 'A', 'A#', 'Bb', 'B'];
  ...
  describe('Data Integrity', () => {
    test('should maintain chord data consistency', () => {
      ...
      expectedKeys.forEach(key => {
        expect(CHORD_MAPS.major[key as KeyType]).toBeDefined();
        expect(CHORD_MAPS.minor[key as KeyType]).toBeDefined();
        expect(CHORD_MAPS.major[key as KeyType]).toHaveLength(7);
        expect(CHORD_MAPS.minor[key as KeyType]).toHaveLength(7);
      });
    });
